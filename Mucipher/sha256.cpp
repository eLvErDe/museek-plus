...
static inline uint32 RORuint32(uint32 x, uint32 y)
{
	return (x >> y) | (x << (32 - y));
}

#define e0(x)       (RORuint32(x, 2) ^ RORuint32(x,13) ^ RORuint32(x,22))
#define e1(x)       (RORuint32(x, 6) ^ RORuint32(x,11) ^ RORuint32(x,25))
#define s0(x)       (RORuint32(x, 7) ^ RORuint32(x,18) ^ (x >> 3))
#define s1(x)       (RORuint32(x,17) ^ RORuint32(x,19) ^ (x >> 10))
...
static inline void LOAD_OP(int I, uint32 *W, const unsigned char *input)
{
	uint32 t1 = input[(4 * I)] & 0xff;
...
static inline void BLEND_OP(int I, uint32 *W)
{
	W[I] = s1(W[I-2]) + W[I-7] + s0(W[I-15]) + W[I-16];
}

static void sha256_transform(uint32 *state, const unsigned char *input)
{
